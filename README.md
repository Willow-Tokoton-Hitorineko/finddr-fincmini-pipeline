...
队名 DeepSeek Your Report · Test Set Rank <b>#12</b>（121.92）
...
**Test Set Rank #12**（队名 DeepSeek Your Report / 模型 FinCMini Agent）。  
名次见 [复旦新闻稿排名附图](https://cs.fudan.edu.cn/93/0f/c24256a758543/page.htm) 或本仓 [测试集最终榜单截图](docs/leaderboard-test-set-final.png)（Test Set 最终榜）；参赛为参赛证明（前三名为获奖证书）。

本仓库为 FinDDR 参赛归档，非比赛官方交付包。

→ [个人贡献说明](docs/contribution.md) · [赛制与项目背景](docs/competition-context.md) · [项目摘要](docs/project-summary.md)
...
| | |
|:--|:--|
| **方案设计及主导推进** | FinCMini Agent：规则库 + Pipeline + LLM（[个人贡献说明 · 方案背景](docs/contribution.md#方案背景)） |
...
**入口文档：** [Pipeline 3.6 说明](pipeline/README_3.6.md) · [地区规范总索引](pipeline/地区规范/README_地区规范总索引.md)
...
1. 从 [Pipeline 3.6 说明](pipeline/README_3.6.md) 了解 Pipeline 3.6
2. 按地区打开 `pipeline/地区规范/` 对应规范（见 [地区规范总索引](pipeline/地区规范/README_地区规范总索引.md)）
3. 对照 `samples/sample00X.md` 与 `outputs/` 里的报告样例
4. （可选）[评分脚本说明](scoring/README.md) — 示例路径需自备数据
